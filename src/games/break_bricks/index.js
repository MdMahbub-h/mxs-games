  ...
  useLayoutEffect(() => {
    const resize = (ev) => {
      if (game.current) {
        ...
        if (canvas) {
          let padding = window.innerWidth > 560 && window.innerHeight > 710 ? 40 : 10;
          ...
          if (winRatio < gameRatio) {
            let canvasWidth = winWidth > 710 ? 710 : winWidth;
            let canvasHeight = winWidth / gameRatio > 560 ? 560 : winWidth / gameRatio;

            canvas.style.width = canvasWidth + "px";
            canvas.style.height = canvasHeight + "px";
          } else {
            let canvasWidth = winHeight * gameRatio > 710 ? 710 : winHeight * gameRatio;
