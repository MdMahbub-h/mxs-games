...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.reset();
    this.add.image(10, 0, "background").setOrigin(0, 0);
  ...
  addNextPieceImage(next_piece_name) {
    this.nextPieceImage = this.add.image(this.scale.width - 15, 70, next_piece_name).setOrigin(1, 0);
  ...
  drawGui() {
    ...
    this.scoreText = this.add
      .text(this.scale.width - 60, 12, "Score: " + this.score, {
        font: "bold 30px Geneva",
        color: "white",
      })
      .setDepth(12)
      .setOrigin(1, 0);
  ...
  incrSpeed() {
    ...
    if (this.score >= this.gameData.winScore) {
      this.gameOver = true;
      this.scene.stop();
